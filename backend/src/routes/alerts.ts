...
export async function alertRoutes(app: FastifyInstance, opts: RouteOptions) {
  app.get("/", async () => {
    if (!opts.prisma) return { alerts: DEFAULT_ALERTS };
    const alerts = await opts.prisma.alert.findMany({
      orderBy: { createdAt: "desc" },
      take: 50,
    });
    if (!alerts.length) return { alerts: DEFAULT_ALERTS };
    return {
  ...
  app.put("/:id/read", async (request, reply) => {
    ...
    if (!opts.prisma) {
      ...
      if (!alert) return reply.status(404).send({ error: { code: "ALERT_NOT_FOUND", message: "Alert not found" } });
      alert.read = true;
      return { success: true };
    }
    try {
