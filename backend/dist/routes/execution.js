...
function estimateFragments(amount, fragMode) {
    if (!fragMode)
        return 1;
    if (amount > 1_000_000)
        return 5;
...
function estimateConfidence(amount, mevGuard) {
    const base = 94;
...
export async function executionRoutes(app, opts) {
    ...
    app.post("/execute", async (request, reply) => {
        ...
        const emitStatus = (stage, status, progress, extra = {}) => {
            publish("execution", {
